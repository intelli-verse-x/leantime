            ...
            <?php if ($login::userIsAtLeast($roles::$editor)) { ?>
                <div class="uploadWrapper">
                    ...
                    <div class="fileUploadDrop">
                        ...
                        <a href="javascript:void(0);" id="addLinkToggle"><?= $tpl->__('label.attach_link'); ?></a>

                        <form id="inlineLinkForm" action="<?= BASE_URL ?>/files/browse" method="post" class="formModal" style="display:none; margin-top:10px;">
                            <input type="hidden" name="upload" value="1" />
                            <input type="hidden" name="uploadType" value="link" />
                            <div style="display:flex; gap:8px; justify-content:center; align-items:center; flex-wrap:wrap;">
                                <input type="url" name="linkUrl" placeholder="https://example.com/document" required style="min-width:280px; width:520px;" />
                                <input type="submit" class="button" value="<?php echo $tpl->__('label.attach_link'); ?>" />
                            </div>
                        </form>
                    </div>
...
<script type='text/javascript'>
    jQuery(document).ready(function() {
        ...
        jQuery('#addLinkToggle').on('click', function() {
            jQuery('#inlineLinkForm').slideToggle(120);
        });
    });
</script>
        ...
        uppy.on('upload-success', (file, response) => {
            ...
            if (response.hasOwnProperty("moduleId")) {
                ...
                let html = '<li class="file-module-' + response.moduleId + '">' +
                    ...
                    <?php
                    if ($login::userIsAtLeast($roles::$editor)) { ?> '<li><a href="<?= BASE_URL ?>/files/browse?delFile=' + response.fileId + '" class="delete deleteFile"><i class="fa fa-trash"></i> <?php echo str_replace("'", '"', $tpl->__('links.delete')); ?></a></li>' +
                    <?php } ?> '</ul>' +
                    '</div>' +
