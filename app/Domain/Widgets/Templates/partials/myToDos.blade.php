...
<div id="yourToDoContainer"
            ...
            <ul class="dropdown-menu pull-right">
                ...
                <li
                    ...
                    @endif><a href=""
                        ...
                        hx-swap="outerHTML"
                        hx-indicator="#todos .htmx-indicator"
                        hx-vals='{"projectFilter": "all", "groupBy": "{{ $groupBy }}" }'>{{ __('labels.all_projects') }}
                ...
                <li
                    ...
                    @endif><a href=""
                        ...
                        hx-swap="outerHTML"
                        hx-indicator="#todos .htmx-indicator"
                        hx-vals='{"projectFilter": "{{ $project['id'] }}", "groupBy": "{{ $groupBy }}" }'>{{ $project['name'] }}</a></li>
                @endforeach
                @endif
    ...
    {{-- Weekly Plan Tasks --}}
    <div id="my-plan-tasks-label"
        style="border-bottom:1px solid var(--main-border-color); padding-bottom:6px; margin-bottom:6px; display:none;">
        <i class="fa fa-calendar-week" style="color:var(--accent1);"></i>
        <strong style="font-size:var(--base-font-size);">{{ __('weeklyplanning.sections.this_weeks_plan') }}</strong>
    </div>
    <div id="my-plan-tasks-wrapper"
        hx-get="{{ BASE_URL }}/hx/weekly-planning/myPlanTasks/get"
                        ...
                        <div>
                            <select name="projectId">
                                ...
                                <option value="{{ $project['id'] }}"
                                    {{ (session('currentProject') == $project['id']) ? 'selected' : '' }}>{{ $project['name'] }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <input type="hidden" name="dateToFinish" value="{{ date('Y-m-d', strtotime('next friday')) }}" />
                        </div>
                        <div>
                            <input type="submit" value="{{ __('buttons.save') }}" name="create" class="btn btn-primary" />
                            <a href="javascript:void(0);" class="btn cancel-add-task" data-group="emptyGroup">{{ __('buttons.cancel') }}</a>
                        </div>
                    </div>
                </form>
            </div>
                                ...
                                <div>
                                    <select name="projectId">
                                        ...
                                        <option value="{{ $project['id']  }}"
                                            ...
                                            {{ (($groupBy === "project" && $project['id'] == $groupKey) || ($groupBy !== "project" && session('currentProject') == $groupKey)) ? 'selected' : '' }}>{{ $project["name"]  }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div>
